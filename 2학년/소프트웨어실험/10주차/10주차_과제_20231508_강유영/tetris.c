...
int main(){
    ...
    //10주차 과제
    Root = (KNode*)malloc(sizeof(KNode));
    Root->lv = -1;
    Root->score = 0;
    Root->f=field;
    Tree(Root);
...
void recommendedPlay(){
    ...
    int i, j;
    start = time(NULL); //start 값을 얻기 위함
    clear();
    act.sa_handler = BlockDown2;
    sigaction(SIGALRM,&act,&oact);
    ...
    do {
        ...
        if(GetCommand()==QUIT){
            alarm(0);
            DrawBox(HEIGHT/2,WIDTH/2-5,1,10);
...
int modified_recommend(KNode* root, int lv){
    //10주차 과제
    int max = 0, i = 0, j, k;
    int t;
    int side = 0, down = 0;
    ...
    {
        if (r>0 && nextBlock[lv] == 4) break;
        else if (r>0 && nextBlock[lv] == 0) break;
        else if (r>0 && nextBlock[lv] == 5) break;
        else if( r>0 && nextBlock[lv] == 6) break;
        ...
        for (int x = -3; x< WIDTH; x++){
            ...
            if (CheckToMove(root->f, nextBlock[lv], r, y, x)){
                ...
                while (CheckToMove(c[i]->f, nextBlock[lv], r, y + 1, x)) y=y+1;
                ...
                c[i]->score = root->score + AddBlockToField(c[i]->f,nextBlock[lv],r,y,x)+5*DeleteLine(c[i]->f)+5*y+3*side-3*down;

                //space 값을 얻기 위함
                space += sizeof(*(c[i]));

                if (c[i]->score - root->score <= 10) continue;

                if (lv == VISIBLE_BLOCKS - 1) max = c[i]->score;
                else{
                    if (lv >= 2 && c[i]->score < (double)max*0.2) continue;

                    t = modified_recommend(c[i], lv + 1);
                    ...
                    else if (max == t && lv == 0 && Root->recBlockY < y){
                        max = t;
...
void Tree(KNode* root){
    KNode **child=root->c;

    for(int i=0;i<CHILDREN_MAX;++i)
    {
        child[i]=(KNode*)malloc(sizeof(KNode));
        child[i]->lv=root->lv+1;
        child[i]->f=(char(*)[WIDTH])malloc(sizeof(char)*HEIGHT*WIDTH);

        // 자식 노드의 레벨을 확인해 재귀적으로 Tree 함수 호출
        if(child[i]->lv<VISIBLE_BLOCKS-1) Tree(child[i]);
    }
}
