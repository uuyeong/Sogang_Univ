...
enum STATIC_OBJECT_ID {
	STATIC_OBJECT_BUILDING = 0, STATIC_OBJECT_WOODTOWER, STATIC_OBJECT_CAT,
	STATIC_OBJECT_DRAGON, STATIC_OBJECT_BUS,
	STATIC_OBJECT_OPTIMUS, STATIC_OBJECT_NEW_PICTURE, STATIC_OBJECT_COW
};

enum DYNAMIC_OBJECT_ID {
	DYNAMIC_OBJECT_TIGER = 0, DYNAMIC_OBJECT_WOLF_1, DYNAMIC_OBJECT_WOLF_2
};
...
struct Shader_Gouraud : Shader {
	...
	GLint loc_eyeLightPos;
	GLint loc_worldLightOn;
	GLint loc_eyeLightOn;
	GLint loc_material;    
	GLint loc_modelLightPos;
	void prepare_shader();
};
...
struct Dynamic_Geometry_Data {
	...
	Wolf_D wolf_d_1{ DYNAMIC_OBJECT_WOLF_1 };
	Wolf_D wolf_d_2{ DYNAMIC_OBJECT_WOLF_2 };
};
...
struct Scene {
	...
	GLboolean world_light_on;
	glm::vec3 world_light_pos;

	GLboolean eye_light_on;

	GLuint tex_bus, tex_wolf;
	bool use_nearest_filter;

	GLboolean model_light_on;

	bool transparent_mode = false; 
	float transparent_alpha = 1.0f;
