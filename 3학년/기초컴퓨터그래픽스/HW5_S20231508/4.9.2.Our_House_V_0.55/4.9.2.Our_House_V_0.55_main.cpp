...
void keyboard(unsigned char key, int x, int y) {
	...
	switch (key) {
		...
		case 'v':
			  // 토글 v 모드
				scene.mode_v_flag = !scene.mode_v_flag;
			if (scene.mode_v_flag) {
				glEnable(GL_DEPTH_TEST);
				glDisable(GL_CULL_FACE);				
			}
			else {				
		...
		case '2':
			...
			break;
		case '`':  // 백틱 키
			scene.shader_kind = SHADER_SIMPLE;   // 다시 기본(Simple) 쉐이더로
			fprintf(stdout, "^^^ Dragon shader reset to SIMPLE.\n");
			break;
		case '3':
			scene.world_light_on = !scene.world_light_on;
			fprintf(stdout, "^^^ World light %s (pos=(125,100,30))\n",
			scene.world_light_on ? "ON" : "OFF");
			glutPostRedisplay();
			break;
		case '4':
			scene.eye_light_on = !scene.eye_light_on;
			fprintf(stdout,
				"^^^ Eye-light %s (camera-relative)\n",
				scene.eye_light_on ? "ON" : "OFF");
			glutPostRedisplay();
			break;
		case '5':
			scene.model_light_on = !scene.model_light_on;
			fprintf(stdout,
				"^^^ Model-light %s (tiger-relative)\n",
				scene.model_light_on ? "ON" : "OFF");
			glutPostRedisplay();
			break;
		case '6':
			scene.transparent_mode = !scene.transparent_mode;
			fprintf(stdout, "Transparent mode %s (Optimus only)\n",
			scene.transparent_mode ? "ON" : "OFF");
			 glutPostRedisplay();
			break;


		case 'N':
			scene.use_nearest_filter = true;
			glBindTexture(GL_TEXTURE_2D, scene.tex_bus);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
			glBindTexture(GL_TEXTURE_2D, scene.tex_wolf);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
			break;
		case 'L':
			scene.use_nearest_filter = false;
			glBindTexture(GL_TEXTURE_2D, scene.tex_bus);
...
void special(int key, int x, int y) {
	switch (key) {
	case GLUT_KEY_F1: currentCam = 1; glutPostRedisplay(); return;
	case GLUT_KEY_F2: currentCam = 2; glutPostRedisplay(); return;
	case GLUT_KEY_F3: currentCam = 3; glutPostRedisplay(); return;
	case GLUT_KEY_F4: currentCam = 4; glutPostRedisplay(); return;
	case GLUT_KEY_F5: currentCam = 5; glutPostRedisplay(); return;
	case GLUT_KEY_F6: currentCam = 6; glutPostRedisplay(); return;
	case GLUT_KEY_F7: currentCam = 7; glutPostRedisplay(); return;
	case GLUT_KEY_UP:
		scene.transparent_alpha = glm::clamp(scene.transparent_alpha + 0.05f, 0.0f, 1.0f);
		fprintf(stdout, "^^^ Alpha = %.2f\n", scene.transparent_alpha);
		break;
	case GLUT_KEY_DOWN:
		scene.transparent_alpha = glm::clamp(scene.transparent_alpha - 0.05f, 0.0f, 1.0f);
		fprintf(stdout, "^^^ Alpha = %.2f\n", scene.transparent_alpha);
