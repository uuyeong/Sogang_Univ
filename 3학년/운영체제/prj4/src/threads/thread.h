...
struct thread
  {
    ...
    int64_t wakeup_tick;                /* Tick to wake up (for sleeping). */
    ...
    int recent_cpu;                     /* Recent CPU for MLFQS (fixed-point). */
    ...
    struct list children_threads;       /* List of child processes. */
    struct list_elem child_elem;        /* List element for child list. */
    struct semaphore child_lock;        /* Semaphore for child synchronization. */
    struct semaphore remove_lock;       /* Semaphore for remove synchronization. */
    
    /* File descriptor table. */
    struct file *fd_table[128];         /* File descriptor table (fd 0,1 reserved). */
    int next_fd;                        /* Next available file descriptor. */
    struct file *executable;            /* Running executable file. */
    
    /* Process loading and execution. */
    bool load_success;                  /* Whether process loaded successfully. */
...
void thread_add_donation (struct thread *donor, struct lock *lock);
