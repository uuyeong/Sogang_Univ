#include "list.h"
#include <assert.h>	
#include <time.h> //따로 추가했다
#include <stdlib.h> //따로 추가했다
...
void list_swap(struct list_elem *a, struct list_elem *b) {
    if (a == b || a == NULL || b == NULL) return;  // 동일 노드이거나 NULL이면 무시
...
void list_shuffle(struct list *list) {
    ...
    if (count < 2) return;  // 요소가 2개 미만이면 섞지 않음

    // 요소 포인터들을 배열에 저장
    struct list_elem *arr[count];
...
// idx 번째 요소를 반환하는 함수
struct list_elem *list_nth(struct list *lst, size_t idx) {
    struct list_elem *e;
    size_t i = 0;
